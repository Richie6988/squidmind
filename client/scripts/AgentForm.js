...
const AgentForm = {
  ...
  _render() {
    ...
    // Skills are SHARED by default across all agents in this platform
    // (like documents / playbooks in Claude, OpenAI Assistants, etc.).
    // Only TOOLS are gated per-agent — that's what limits capabilities and
    // shrinks the agent's prompt. capabilities.skills used to be a cosmetic
    // per-agent whitelist that only added a line to the prompt without
    // actually restricting anything at runtime. Removed for clarity.
